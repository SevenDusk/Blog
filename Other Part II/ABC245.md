...
首先考虑将$M$分解质因数，令$M=\prod p_i^{r_i}$，那么我们可以将原问题分成若干个子问题（个数为$M$不同质因数个数）

第$i$个子问题形如，对所有长度为$K$的序列$A$计数，要求满足

- $A_i\in [0,p_i^{r_i})$
- $\prod \limits_{i=1}^k A_i\equiv N\ (\bmod p_i^{r_i})$

那么假设我们对所有这些子问题都求出一个解$A$，那么我们可以通过CRT唯一的还原出原来问题的序列（由于每一个子问题的模数都是互质的），那么原问题的答案等于所有子问题答案的乘积，考虑求解一个子问题

首先考虑$N\not \equiv 0(\bmod p^r)$的情况

可以发现的是序列中所有数的$p$因子个数之和要等于$N\bmod p^r$的中$p$的因子个数，那么假设$cnt$为最大的正整数满足$p^{cnt}|(N\bmod p^r)$，可以将这$cnt$个因子$p$分配到$k$个数中，这样的方案数为$\binom{k-1+cnt}{cnt}$

接下来我们就不需要考虑$p$因子的数，可以令$B_i$为$A_i$去掉所有$p$因子之后得到的数，$N'$表示$(N\bmod p^r)$去掉所有$p$因子之后得到的数

那么要求$\prod \limits_{i=1}^k B_i\equiv N'(\bmod p^r)$

- 事实上对于模数为质数的若干次方的情况也是存在原根的，并且$p^k$的原根等于$p$的原根$g$或者$g+p$，也就是说对于任意$i,j\in [0,\varphi(p^k))$满足$g^i\neq g^j$，并且$\forall i\in [0,\varphi(p^k))$满足$p$不整除$g^i$

根据上面的转化可以求出$g^{C_i}=B_i,g^R=N'$，那么限制转化为$\sum\limits_{i=1}^k C_i\equiv R(\bmod \varphi(p^r)),C_i\in [0,\varphi(p^r))$

那么由于每一个值都是等价的，那么对$C$的计数就是$\varphi(p^r)^{k-1}$

那么方案数就是$\binom{k-1+cnt}{cnt}\times \varphi(p^r)^{k-1}$



接下来考虑$N\equiv 0(\bmod p^r)$的情况

那么也就是说$A$序列中$p$出现次数需要$\geq r$

- 如果存在$A_i=0$，那么方案数就是$(p^r)^k-(p^r-1)^k$

- 如果所有$A_i> 0$，记$B_i$表示$A_i$中$p$因子的次数，显然只需要关系$B_i>0$的位置即可，由于限制了$A_i\neq 0$，那么说明$B_i\in [0,r)$，考虑进行容斥，求出总方案数-次数和$<r$的方案数即可

  设$f(a,b,c)$表示考虑到前$a$中$B$的取值，当前$p$因子个数和为$b$的方案数，当前涉及了$c$个位置

  那么有转移，其中$B=a$的方案数有$p^{r-a}-p^{r-a-1}$
  $$
  f(a,b,c)=\sum\limits_{i\geq 1,ia\leq b,i\leq c} f(a-1,b-ia,c-i)\times\binom{k-c+i}{i}\times (p^{r-a}-p^{r-a-1})^i
  $$
  那么最终方案数就是
  $$
  (p^r-1)^k-\sum\limits_{i=0}^r \sum\limits_{j=0}^{\min(r,k)} f(r-1,i,j)\times (p^r-p^{r-1})^{k-j}
  $$
...
复杂度$O(\sqrt M+\log^3 M\log \log M)$
...
template <typename T, typename S> T ceil(T x, S y) {//x/y上取整
    assert(y);
    return (y < 0 ? ceil(-x, -y) : (x > 0 ? (x + y - 1) / y : x / y));
}

template <typename T, typename S> T floor(T x, S y) {//x/y下取整
    assert(y);
    return (y < 0 ? floor(-x, -y) : (x > 0 ? x / y : x / y - (x % y == 0 ? 0 : 1)));
}
template <class T> T POW(T x, int n) {//快速幂
    T res = 1;
    for(; n; n >>= 1, x *= x)
        if(n & 1) res *= x;
    return res;
}
template <class T, class S> T POW(T x, S n, const ll &mod) {//带模数的快速幂
    T res = 1;
    x %= mod;
    for(; n; n >>= 1, x = x * x % mod)
...
vector<pll> factor(ll x) {//sqrt n质因数分解
    vector<pll> ans;
    for(ll i = 2; i * i <= x; i++)
        if(x % i == 0) {
            ans.push_back({i, 1});
...
template <class T> vector<T> divisor(T x) {//求出所有因子
    vector<T> ans;
    for(T i = 1; i * i <= x; i++)
        if(x % i == 0) {
            ans.pb(i);
...
Graph getG(int n, int m = -1, bool directed = false, int margin = 1) {//读入一张图
    Tree res(n);
    if(m == -1) m = n - 1;
    while(m--) {
        int a, b;
...
Graph getTreeFromPar(int n, int margin = 1) {//读入树
    Graph res(n);
    for(int i = 1; i < n; i++) {
        int a;
        cin >> a;
...
void add(Graph &G, int x, int y) { G[x].eb(y), G[y].eb(x); }//添加一条边
...
istream &operator>>(istream &is, i128 &v) {//快速输入
    string s;
    is >> s;
    v = 0;
...
ostream &operator<<(ostream &os, const i128 &v) {//快速输出
    if(v == 0) { return (os << "0"); }
    i128 num = v;
    if(v < 0) {
        os << '-';
...
vector<mint> Inv;
mint inv(int x);
template <ll Modulus> class modint {//封装整数类
...
constexpr int primitive_root_constexpr(int m) {//求质数p的原根
    if(m == 2) return 1;
...
void butterfly_inv(std::vector<mint> &a) {//逆向蝴蝶变换
    static constexpr int g = internal::primitive_root<mint::mod()>;
    int n = int(a.size());
    int h = internal::ceil_pow2(n);
...
std::vector<mint> convolution(std::vector<mint> a, std::vector<mint> b) {//多项式DFT
...
Poly inv(const Poly &f, int s = -1) {//多项式求逆
    if(s == -1) s = f.size();
    Poly r(s);
    r[0] = mint(1) / f[0];
    for(int n = 1; n < s; n *= 2) {
...
Poly pow(const Poly &f, ll k, int need = -1) {//多项式快速幂 O(n\log n)
...
struct ArbitraryLazyMontgomeryModInt {//封装整数类 
...
template <typename mint> bool miller_rabin(u64 n, vector<u64> as) {//素性测试
...
template <typename mint, typename T> T pollard_rho(T n) {// n^{1/3}复杂度分解质因数
    if(~n & 1) return 2;
...
vector<u64> factorize(u64 n) {//快速分解质因数
...
int main() {
    ...
    fore(x, y, mp) {
        int c = 0;
        while(n % x == 0 and c < y) n /= x, c++;
        vv(mint, dp, c + 1, c + 1);
        vl v{POW(x, y - 1) * (x - 1)};
        rep(i, y - 1) v.eb(v.back() / x);
        v.eb(1);

        dump(x, y, c, v);

        rep(j, 1, c + 1) dp[1][j] = 1;
        rep(i, 1, c) {
            rep(j, c) {
                rep(k, 1, c + 1) {
                    if(j + k <= c) { dp[i + 1][j + k] += dp[i][j] * POW(x, y - 1) * (x - 1); }
                }
            }
        }
        ...
        if(c < y) {
            ...
            rep(i, c + 1) {
                if(i > N) break;
                s += dp[i][c] * mint(v[0]).pow(N - i) * C(N, i);
            }
            if(c == 0) s += mint(v[0]).pow(N - 1);
            ans *= s;
            dump(s);
        } else {
            mint s = mint(POW(x, y)).pow(N);
            rep(i, c + 1) rep(j, c) {
                if(i > N) break;
                s -= dp[i][j] * mint(v[0]).pow(N - i) * C(N, i) * v[j];
            }
            s -= mint(v[0]).pow(N);
            ans *= s;
            dump(s);
...
收获：封装的基本数论函数，多项式各类操作，$n^{\frac{1}{3}}$的pollard_rho质因数分解，对基本图和树的处理，按照模块进行封装，在写代码的时候可以不让板子的细节干扰到主体思路